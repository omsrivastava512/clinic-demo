...
export const PresentationSection = ({ hidden = false, title, number, description, children, tag, className }: PSProps) => (
    <section className="mb-32 max-w-5xl mx-auto w-full last:mb-0" >
        ...
        <div id={tag} className={cn("p-2 md:p-4 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-900 rounded-2xl shadow-xl transition-colors duration-300", className)} hidden={hidden} aria-hidden={hidden}>
            {children}
        </div>
    </section>
);
