...
const DailyLedger: React.FC<LedgerProps> = ({ onPatientIdentified }) => {
  ...
  return (
    <div className="w-full bg-black border border-zinc-800 rounded-lg overflow-hidden flex flex-col h-[400px]">
      {/* Header */}
      <div className="flex justify-between items-end p-6 border-b border-zinc-900 bg-zinc-950">
        <div>
          <h3 className="text-white font-medium">Daily Register (दैनिक रजिस्टर)</h3>
        ...
        <div className="text-right">
          ...
          <div className="text-xs text-zinc-600 uppercase tracking-widest">OPD Count</div>
        </div>
      </div>

      {/* Table Header */}
      <div className="grid grid-cols-12 gap-4 px-6 py-3 bg-zinc-900 border-b border-zinc-800 text-xs font-medium text-zinc-500 uppercase tracking-wider">
      ...
      <div className="flex-1 overflow-y-auto">
        {/* Mock Past Entries */}
        <div className="grid grid-cols-12 gap-4 px-6 py-4 border-b border-zinc-900 hover:bg-zinc-900/50">
          <div className="col-span-2 text-zinc-500 font-mono text-xs">10:15 AM</div>
          <div className="col-span-4 text-zinc-300 font-medium">Vikram Singh</div>
          <div className="col-span-4 text-zinc-500 text-sm">Frozen Shoulder (IFT + US)</div>
          <div className="col-span-2 text-right"><span className="text-xs bg-emerald-900/30 text-emerald-500 px-2 py-1 rounded">Paid</span></div>
        </div>
        <div className="grid grid-cols-12 gap-4 px-6 py-4 border-b border-zinc-900 hover:bg-zinc-900/50">
          <div className="col-span-2 text-zinc-500 font-mono text-xs">10:30 AM</div>
          <div className="col-span-4 text-zinc-300 font-medium">Priya Kapoor</div>
          <div className="col-span-4 text-zinc-500 text-sm">Ankle Sprain (Taping)</div>
          <div className="col-span-2 text-right"><span className="text-xs bg-zinc-800 text-zinc-400 px-2 py-1 rounded">In Therapy</span></div>
        </div>

        {/* ACTIVE INPUT ROW */}
        <div className="grid grid-cols-12 gap-4 px-6 py-4 bg-zinc-900/20 items-center border-l-4 border-white animate-pulse hover:animate-none transition-all">
          <div className="col-span-2 text-white font-mono text-xs">Now</div>
          <div className="col-span-10 relative">
            ...
            <input 
              ...
              className="w-full bg-transparent border-none outline-none text-xl font-medium text-white placeholder-zinc-600 pl-8"
              placeholder="Type Name (e.g. 'Rajesh') or Mobile..." 
            />
            <div className="absolute right-0 top-1/2 -translate-y-1/2 flex gap-2">
               <span className="hidden sm:inline-block text-[10px] bg-zinc-800 text-zinc-400 px-1.5 py-0.5 rounded border border-zinc-700">↵ ENTER</span>
            </div>
          </div>
        </div>
      </div>
...
const ContextSelector: React.FC<ContextSelectorProps> = ({ patient, availableContexts, onConfirm, onCancel }) => {
  ...
  return (
    <div className="w-full max-w-2xl bg-black border border-zinc-800 rounded-xl p-8 shadow-2xl">
      <div className="flex justify-between items-start mb-6">
        <div>
          <h2 className="text-2xl font-bold text-white">{patient.name}</h2>
          <p className="text-zinc-400 text-sm mt-1">MRN: {patient.mrn} • +91 {patient.phone}</p>
        </div>
          ...
          {availableContexts.map(ctx => {
            ...
            return (
              <div 
                ...
                `}
              >
                <div className={`w-5 h-5 rounded flex items-center justify-center mr-4 transition-colors ${isSelected ? 'bg-white text-black' : 'border border-zinc-600'}`}>
                  {isSelected && <Check className="w-3 h-3" />}
                </div>
                <div className="flex-1">
                  <div className={`font-medium ${isSelected ? 'text-white' : 'text-zinc-400 group-hover:text-zinc-200'}`}>
                    {ctx.title}
                  </div>
                  {ctx.type === 'EXISTING' && (
                    <div className="text-xs text-zinc-600">{ctx.doctor} • Active Plan</div>
                  )}
                </div>
                {ctx.type === 'NEW' && <Plus className="w-4 h-4 text-zinc-600" />}
              </div>
            );
          })}
        </div>
      ...
      <div className="flex justify-end gap-3 pt-6 border-t border-zinc-900">
        ...
        <button 
          ...
          className="px-6 py-2 bg-white text-black font-bold rounded hover:bg-zinc-200 disabled:opacity-50 disabled:cursor-not-allowed"
        >
...
const ProcedureLogger: React.FC<ProcedureLoggerProps> = ({ selectedContexts, onComplete }) => {
  ...
  return (
    <div className="w-full flex flex-col md:flex-row gap-6 h-[600px]">
      ...
      <div className="flex-1 overflow-y-auto pr-2 space-y-8">
        ...
        {selectedContexts.map((ctx, index) => (
          <div key={ctx.id} className="animate-fadeIn" style={{ animationDelay: `${index * 100}ms` }}>
            ...
            {/* Context Header */}
            <div className="flex items-center gap-3 mb-4 sticky top-0 bg-black/95 backdrop-blur py-2 z-10 border-b border-zinc-900">
              ...
              <div>
                <h3 className="text-white font-bold">{ctx.title}</h3>
                <p className="text-xs text-zinc-500">Select procedures for this specific area</p>
              </div>
            </div>

            {/* Grid for THIS Context */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pl-4 border-l-2 border-zinc-800">
                ...
                return (
                  <button
                    ...
                    className={`
                      ...
                      ${isSelected 
                        ? 'bg-zinc-900 border-white text-white shadow-[0_0_15px_rgba(255,255,255,0.05)]' 
                        : 'bg-black border-zinc-800 text-zinc-500 hover:border-zinc-600 hover:bg-zinc-900/50'}
                    `}
                  >
                    <div>
      ...
      <div className="w-full md:w-80 bg-zinc-900/30 border border-zinc-800 rounded-xl p-6 flex flex-col h-full">
        <h4 className="text-xs font-bold text-zinc-500 uppercase tracking-widest mb-4">Current Session Bill</h4>
        
        <div className="flex-1 overflow-y-auto space-y-3">
          {Object.keys(items).length === 0 && (
            <div className="text-center text-zinc-600 text-sm py-10">No procedures selected yet.</div>
          )}
          ...
          {selectedContexts.map(ctx => {
             ...
             return (
               <div key={ctx.id} className="mb-4">
                 <div className="text-[10px] text-zinc-400 font-bold uppercase mb-1 truncate">{ctx.title}</div>
                 {ctxItems.map(item => (
                   <div key={`${item.contextId}-${item.procedureId}`} className="flex justify-between text-sm py-1 border-b border-zinc-800 last:border-0">
        ...
        <div className="border-t border-zinc-700 pt-4 mt-4">
          <div className="flex justify-between items-center mb-4">
            ...
            <span className="text-2xl font-bold text-white font-mono">₹{totalCost}</span>
          </div>
          <button 
...
const InvoiceBuilder: React.FC<InvoiceBuilderProps> = ({ items, patientName, onClose }) => {
  ...
  return (
    <div className="w-full max-w-lg bg-black border border-zinc-800 rounded-xl overflow-hidden shadow-2xl mx-auto">
      {/* Receipt Header */}
      <div className="bg-zinc-900 p-6 border-b border-zinc-800 flex justify-between items-start">
        <div>
          <h2 className="text-white font-bold text-lg">Invoice #INV-2024-88</h2>
      ...
      <div className="p-6">
        {/* Itemized List */}
        <div className="bg-zinc-950 rounded-lg p-4 mb-6 border border-zinc-900">
              ...
              {items.map((item, i) => (
                <tr key={i}>
                  <td className="py-2">
                    ...
                    <div className="text-[10px] text-zinc-600">{item.contextName}</div>
                  </td>
        ...
        <div className="mb-8">
          ...
          <div className="grid grid-cols-3 gap-3">
            <button 
              ...
              className={`flex flex-col items-center justify-center p-3 rounded border transition-all ${paymentMode === 'UPI' ? 'bg-zinc-800 border-white text-white' : 'border-zinc-800 text-zinc-500 hover:bg-zinc-900'}`}
            >
            ...
            <button 
              ...
              className={`flex flex-col items-center justify-center p-3 rounded border transition-all ${paymentMode === 'CASH' ? 'bg-zinc-800 border-white text-white' : 'border-zinc-800 text-zinc-500 hover:bg-zinc-900'}`}
            >
            ...
            <button 
              ...
              className={`flex flex-col items-center justify-center p-3 rounded border transition-all ${paymentMode === 'CARD' ? 'bg-zinc-800 border-white text-white' : 'border-zinc-800 text-zinc-500 hover:bg-zinc-900'}`}
            >
        ...
        <div className="flex justify-between items-center pt-4 border-t border-zinc-800">
           <button className="text-zinc-500 hover:text-white flex items-center gap-2 text-sm">
             <Printer className="w-4 h-4" /> Print Receipt
           </button>
           <button 
             onClick={onClose}
             className="px-6 py-2 bg-emerald-700 text-white font-bold rounded hover:bg-emerald-600 transition-colors shadow-[0_0_20px_rgba(4,120,87,0.3)]"
           >
             Confirm Payment
           </button>
        </div>
...
const PresentationSection = ({ title, children }: { title: string, children: React.ReactNode }) => (
  <div className="mb-32 max-w-5xl mx-auto w-full">
    <div className="mb-6 px-4">
      <h2 className="text-2xl font-bold text-white tracking-tight border-l-4 border-zinc-700 pl-4">{title}</h2>
    </div>
    <div className="p-4 md:p-8 bg-zinc-950 border border-zinc-900 rounded-2xl shadow-xl">
      {children}
    </div>
  </div>
);
...
const MedCoreComponentCatalog = () => {
  ...
  const handleLog = (val: unknown) => console.log("Action Triggered:", val);

  return (
    <div className="min-h-screen bg-black text-zinc-100 py-20 px-4 font-sans selection:bg-zinc-800">
      
      <div className="text-center mb-20">
        <h1 className="text-4xl md:text-6xl font-bold tracking-tighter text-white mb-4">MedCore Components</h1>
        <p className="text-zinc-500">Modular React + TS components for Indian Physiotherapy Clinics.</p>
        <p className="text-zinc-600 text-sm mt-2 font-mono">Theme: Pearl Black • Mode: Many-to-Many Context</p>
      </div>
